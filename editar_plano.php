...
<form action="processa_edicao_plano.php" method="POST">
    ...
    <div>
        ...
        <input type="number" name="valor" step="0.01" min="0" 
               value="<?php echo $plano['valor']; ?>" required>
    </div>

    <div style="display: flex; gap: 20px;">
        <div style="flex: 1;">
            <label for="sessoes_contratadas">Sessões Totais:</label>
            <input type="number" name="sessoes_contratadas" 
                   value="<?php echo $plano['sessoes_contratadas']; ?>" min="1" required>
        </div>
        <div style="flex: 1;">
            <label>Já Utilizadas:</label>
    ...
    <div>
        ...
        <select id="status" name="status" required>
            <option value="Ativo" <?php if($plano['status'] == 'Ativo') echo 'selected'; ?>>Ativo</option>
            <option value="Concluido" <?php if($plano['status'] == 'Concluido') echo 'selected'; ?>>Concluído</option>
        </select>
    </div>
